...
let _options =
  [
    { key: 'Header', text: 'List Items', itemType: DropdownMenuItemType.Header },
  ];
      ...
      <Dropdown
        ...
        <div className={`  `}>
            ...
            <input type="file" onChange={(e) => this.handleFileUpload(e.target)}  />
              <PrimaryButton className={`${styles.button}`} onClick={() => this.uploadattach(optionkey)}>
                <span className={styles.label}>Upload Report</span>
              </PrimaryButton>
            </div>
          </div>
      </div>
...
private uploadattach(optionkey): void {
  ...
  if(!optionkey)
    console.log("no option key show dialog box");
